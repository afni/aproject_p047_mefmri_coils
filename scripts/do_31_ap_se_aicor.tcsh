...
# AP_SE_AICOR: run AP processing with ANATICOR for SE-FMRI data
...
set dir_ap         = ${dir_inroot}/data_31_ap_se_aicor
...
# EPI data (full path); only 1 run here --- only 2nd echo
cd ${sdir_func}
set taskname    = rest
set label       = task-${taskname}
set dset_epi    = `find ${sdir_func} -name "${subjid}*${label}*echo-2*bold.nii*" \
                      | sort`
set json_epi    = `find ${sdir_func} -name "${subjid}*${label}*echo-2*bold.json" \
                        | sort`
...
# Comments on processing, SE-FMRI data
#
# + input EPI dsets have 3.0 mm isotropic voxels, determining final size
# + for blur for this voxelwise analysis of SE EPI, blur at ~1.5-2x
#   input voxel dim (and no sinc interp on volreg_warp
# + input EPI have pre-steady still, so remove a couple time points at start
# + try using ANATICOR with WM maask from FS to remove coil artifact (not
#   adding in ventricle PCs here)
...
afni_proc.py                                                                  \
    ...
    -anat_follower_ROI         FSWe epi ${sdir_suma}/fs_ap_wm.nii.gz          \
